...
int HyperNEAT_main(int argc,char **argv)
{
	...
	try
	{
		if(commandLineParser.HasSwitch("-ORG")){
			...
			org1 = cmdLine.GetArgument("-ORG", 0);
			org2 = cmdLine.GetArgument("-ORG", 1);
			//test org1 and org2
			// if org1 and org2 are absent -> Individual
			if (org1.empty() && org2.empty()) cout << "new individual";
			// if org2 is absent -> Individual + Mutation
			if (!org1.empty() && org2.empty()) cout << "mutated individual";
			// if org1 and org2 are nonempty -> parents Xover plus mutation
			if (!org1.empty() && !org2.empty()) cout << "new offspring";
		}
