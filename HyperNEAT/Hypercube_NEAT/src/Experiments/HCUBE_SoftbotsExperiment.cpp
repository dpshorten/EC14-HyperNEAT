...
namespace HCUBE {
    ...
    double SoftbotsExperiment::processEvaluation(shared_ptr<NEAT::GeneticIndividual> individual, string individualID, int genNum, bool saveVxaOnly, double bestFit) {
        ...
        for (int j = 0; j < ContinuousArray.GetFullSize(); j++) //iterate through each location of the continuous matrix
        {
            ...
            if (addAngularPositionXY) {
                if (yNormalized == 0.0 and xNormalized == 0.0) {
                    angularPositionXY = 0.0;
                } else {
                    angularPositionXY = atan2(double(yNormalized), double(xNormalized)) / PI;
                }
            }
            if (addAngularPositionYZ) {
                if (yNormalized == 0.0 and zNormalized == 0.0) {
                    angularPositionYZ = 0.0;
                } else {
                    angularPositionYZ = atan2(double(zNormalized), double(yNormalized)) / PI;
                }
            }
            if (addAngularPositionXZ) {
                if (zNormalized == 0.0 and xNormalized == 0.0) {
                    angularPositionXZ = 0.0;
                } else {
                    angularPositionXZ = atan2(double(zNormalized), double(xNormalized)) / PI;
                }
            }
        ...
        if (evolvingSensorsAndMotors) {
            ...
            for (int j = 0; j < ContinuousArray.GetFullSize(); j++) //iterate through each location of the continuous matrix
            {
                if (SensorArrayForVoxelyze[j] > numSensors) {
                    numSensors = SensorArrayForVoxelyze[j];
                }
                if (MuscleArrayForVoxelyze[j] > numMuscles) {
            ...
            if (numSensors == 0) {
                ...
                return 0.00001;
            } // exit if no muscles or sensors
            if (numMuscles == 0) {
                cout << "No Muscles... assigning fitness of 0.00001" << endl;
                return 0.00001;
            } // exit if no muscles or sensors
            if (numSensors > 99) {
                cout << "Over 100 Sensors... assigning fitness of 0.00001" << endl;
                return 0.00001;
            } // exit if no muscles or sensors
            if (numMuscles > 99) {
                cout << "Over 100 Muscles... assigning fitness of 0.00001" << endl;
                return 0.00001;
            } // exit if no muscles or sensors
        ...
        for (int j = 0; j < ContinuousArray.GetFullSize(); j++) //iterate through each location of the continuous matrix
        {
            if (ArrayForVoxelyze[j] > 0) {
                numTotalVoxels++;
            }
            if (SensorArrayForVoxelyze[j] > 0) {
                numSensorVoxels++;
            }
            if (MuscleArrayForVoxelyze[j] > 0) {
                numMuscleVoxels++;
            }
        }

        if (numTotalVoxels < 2) {
            cout << "No Voxels Filled... setting fitness to 0.00001" << endl;
            return 0.00001;
        }
        ...
        if (true) {
            ...
            if (!pipe) {
                cout << "ERROR 1, exiting." << endl << endl;
                return 0.00001;
            }
            char buffer[128];
            std::string result = "";
            ...
            } else
 {
            ...
            if (int(NEAT::Globals::getSingleton()->getParameterValue("PenaltyType")) != 0) {
                fitness = fitness * (voxelPenaltyf);
            }
            //cout << "Fitness Penalty Adjustment: " << voxelPenaltyf << endl;
            if (int(NEAT::Globals::getSingleton()->getParameterValue("PenaltyType")) != 0) {
                printf("Fitness Penalty Multiplier: %f\n", voxelPenaltyf);
            }
    ...
    int SoftbotsExperiment::writeVoxelyzeFile(CArray3Df ArrayForVoxelyze, CArray3Df SensorArrayForVoxelyze, CArray3Df MuscleArrayForVoxelyze, string individualID, float synapseWeights [100000], float sensorPositions [100][3], float hiddenPositions [100][3], float musclePositions [100][3], int numSensors, int numHidden, int numMuscles, float nodeBias [300], float nodeTimeConstant [300], shared_ptr<NEAT::GeneticIndividual> individual) {
        ...
        int ObsBuf;
        if (DoObs > 0) ObsBuf = 50;
        else ObsBuf = 0; //we'll add 50 voxels around the object to allow space (should really make this value a parameter) 
...
<TempPeriod>0.025</TempPeriod>\n";
        if (evolvingNeuralNet) {
            myfile << "<EvolvingNeuralNetworks>1</EvolvingNeuralNetworks>\n";
        } else {
            myfile << "<EvolvingNeuralNetworks>0</EvolvingNeuralNetworks>\n";
        }
        if (phaseDrivenActuation) {
            myfile << "<PhaseDrivenActuation>1</PhaseDrivenActuation>\n";
        } else {
            myfile << "<PhaseDrivenActuation>0</PhaseDrivenActuation>\n";
        }
        myfile << "\
</Thermal>\n\
</Environment>\n\
...
";
                ...
                for (int x = 0; x < num_x_voxels; x++) {
                    if (evolvingSensorsAndMotors) {
                        if (ContinuousArray[v] < 0) {
                            ...
                            if (PassiveStiffArray[v] == maxVal) {
                                ArrayForVoxelyze[v] = 2;
                            }
                            if (PassiveSoftArray[v] == maxVal) {
                                ArrayForVoxelyze[v] = 1;
                            }
                        }
                    }
                    else // notEvolvingSensorsAndMotors == muscle phases
                    {
                        if (ContinuousArray[v] < 0) {
                            ...
                            // 	{ ArrayForVoxelyze[v] = 2;}
                            double maxVal = std::max(std::max(PassiveSoftArray[v], PassiveStiffArray[v]), std::max(Phase1Array[v], Phase2Array[v]));
                            if (Phase2Array[v] == maxVal) {
                                ArrayForVoxelyze[v] = 4;
                            }
                            if (Phase1Array[v] == maxVal) {
    ...
    int SoftbotsExperiment::forwardExists(CArray3Df ArrayForVoxelyze, int currentPosition) {
        if (int(currentPosition / num_x_voxels) % num_y_voxels == num_y_voxels - 1) return -999;
            ...
            numConnections++;
            return (currentPosition + num_x_voxels);
        }
    }
    ...
    int SoftbotsExperiment::upExists(CArray3Df ArrayForVoxelyze, int currentPosition) {
        if (int(currentPosition / (num_x_voxels * num_y_voxels)) % num_z_voxels == num_z_voxels - 1) return -999;
            ...
            numConnections++;
            return (currentPosition + num_x_voxels * num_y_voxels);
        }
    }
    ...
    bool SoftbotsExperiment::isNextTo(int posX1, int posY1, int posZ1, int posX2, int posY2, int posZ2) {
        // bool nextTo = true;
        if (abs(posX1 - posX2) > 1) {
            return false;
        }
        if (abs(posY1 - posY2) > 1) {
            return false;
        }
        if (abs(posZ1 - posZ2) > 1) {
            return false;
        }
        return true;
    }
